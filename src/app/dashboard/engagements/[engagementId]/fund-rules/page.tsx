...
export default async function FundRulesPage({
  params,
}: {
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <Card>
        <CardHeader>
          <CardTitle>Fund Detection Rules</CardTitle>
        </CardHeader>

        <CardContent className="space-y-4">
          <p className="text-sm text-gray-600">
            Clients use many account formats (10-xxxx, 10.4000, 10-50-4000, 10-50400.1, 1040000, etc). We detect the
            fund with flexible regex rules; the fund code comes from a capture group (usually group 1).
          </p>

          <form action={rerunFundDetection} className="flex gap-2">
            <input type="hidden" name="engagementId" value={params.engagementId} />
            <Button type="submit" variant="secondary">
              Re-run detection on latest TB
            </Button>
          </form>
                  ...
                  <tr key={r.id} className="border-t">
                    <td className="px-3 py-2">
                      <form action={toggleFundRule}>
                        ...
                        <Button type="submit" variant={r.enabled ? "default" : "secondary"}>
                          {r.enabled ? "On" : "Off"}
                        </Button>
                      </form>
                    </td>
            ...
            <form action={createFundRule} className="grid gap-3 md:grid-cols-3">
              ...
              <div className="md:col-span-2">
                ...
                <div className="text-xs text-gray-500 mt-1">
                  Tip: put fund code in capture group 1 like{" "}
                  <span className="font-mono">{String.raw`^(\d{2})`}</span>.
                </div>
              </div>
