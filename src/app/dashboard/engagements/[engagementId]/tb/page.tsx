...
export default async function TBPage({ params }: { params: { engagementId: string } }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader><CardTitle>Latest Import Preview</CardTitle></CardHeader>
        <CardContent>
          {!preview ? <p className="text-sm text-gray-500">No import yet.</p> : (
            <div className="space-y-4">
              <div className="grid gap-2 md:grid-cols-3">
                <div className="text-sm"><span className="text-gray-500">File:</span> <span className="font-medium">{preview.filename}</span></div>
                <div className="text-sm"><span className="text-gray-500">Rows:</span> <span className="font-medium">{preview.rowCount}</span></div>
                <div className="text-sm"><span className="text-gray-500">Total:</span> <span className="font-medium">{Number(preview.totalBalance).toLocaleString(undefined,{minimumFractionDigits:2, maximumFractionDigits:2})}</span></div>
              </div>
                  ...
                  <tbody>
                    {preview.lines.map(l => (
                      <tr key={l.id} className="border-t">
                        ...
                        <td className="px-3 py-2 text-right">{Number(l.finalBalance).toLocaleString(undefined,{minimumFractionDigits:2, maximumFractionDigits:2})}</td>
