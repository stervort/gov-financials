...
def read_csv_preview(csv_text: str, has_headers: bool = True, header_row: int = 1, delimiter: str = ",", max_rows: int = 50):
    """
    Returns (headers, rows) where rows are list[dict] if headers present, else list[list].
    """
    ...
    if has_headers:
        headers = [h.strip() if h else "" for h in first]
        rows = []
